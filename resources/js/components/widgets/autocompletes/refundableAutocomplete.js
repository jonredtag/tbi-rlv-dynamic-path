...
class RefundableAutocomplete extends Component {
    constructor(props) {
        super(props);
    ...
    filter(value) {
        ...
        xhr.open('GET', `/api/dynamic/${autocompleteType}/${encodeURIComponent(`${departure ? `${departure}||` : ''}${value}`)}${sid ? `?sid=${sid}` : ''}`);
...
RefundableAutocomplete.propTypes = {
...
RefundableAutocomplete.defaultProps = {
    onVisibilityChange: () => {},
    inputClass: '',
    icon: 'icon',
};

export default autocompleteHOC(RefundableAutocomplete);
